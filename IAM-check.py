...
# Use g4 to list the files in the CL
cl = input("Enter your CL number: ")
...
def print_wildcards(checks):
  ...
  print_found_problems(checks, "wildcards")


# Reads a file and split its contents into lines
def read_lines(file):
  with open(file, 'r') as f:
    return f.read().splitlines()

# Split and index (start/end) all the exemption blocks
def split_blocks(content):
  ...
  sub_sIndex, sub_eIndex = None, None
  subblock_flag = False
  exemption_flag = False

  # Loop through the lines of the file and find the start and end indices of each exemption block
  for index, c in enumerate(content):
    if 'exemption' in c and not exemption_flag:
      sIndex = index
      exemption_flag = True
    elif 'TODO' in c:
        sIndex = index
        exemption_flag = True
    elif '#' in c and 'TODO' not in c: continue
    elif '}' in c:
      # Check if it is a closing bracket for exemption, or for subblock
      if subblock_flag:
        sub_eIndex = index
        subblock_flag = False
        ...
        exemption_flag = False
        eIndex = index
        iterators.append({'sIndex': sIndex, 'eIndex': eIndex, 'sub_sIndex': sub_sIndex, 'sub_eIndex': sub_eIndex})
    else:
      # Check for subblocks
      if 'conditional_account' in c:
        sub_sIndex = index
        subblock_flag = True
...
# Extracts all the accounts from exemption blocks
...
def check_duplicate_accounts(blocks, accounts, unique, index):
  ...
  if index == 0:
    if len(accounts) <= 0: accounts.append('')
...
# Remove duplicate exemption blocks
def remove_duplicates(blocks, accounts):
  ...
  for i, block in enumerate(blocks[0:-1]):
    if blocks[i] == blocks[i+1]:
      ...
      # Sometimes blocks don't have any accounts, so we need to match len() of both lists
...
def sort_lines_in_blocks(blocks):
  for i in range(len(blocks)):
    ...
    # TODO comments and blocks start
    exemptions = [line for line in blocks[i] if line.strip().startswith('exemption')]
    todos = [line for line in blocks[i] if line.strip().startswith('# TODO')]
    ...
    acls = permissions + roles + accounts + scopes

    # `conditional_account_type` subblock
    conditionals = [line for line in blocks[i] if line.strip().startswith('conditional_account')]
    account_props = [line for line in blocks[i] if line.strip().startswith('account_property')]
    account_types = [line for line in blocks[i] if line.strip().startswith('account_type')]
    ...
    blocks[i] = headers + acls + subblocks
